...
class ModelTrainer(object):
    ...
    def create_model(self, depth, neurons):
        self.model = Sequential()
        self.model.add(Conv2D(neurons[0], (3, 3), padding='same', input_shape=self.X.shape[1:]))
        ...
        if depth > 1:
            ...
            if depth > 2:
                self.model.add(Conv2D(neurons[1], (3, 3), padding='same'))
                self.model.add(Activation('relu'))
                ...
                if depth > 3:
                    ...
                    if depth > 4:
                        self.model.add(Conv2D(neurons[2], (3, 3), padding='same'))
                        self.model.add(Activation('relu'))
                        ...
                        if depth > 5:
                            ...
                            if depth > 6:
                                self.model.add(Conv2D(neurons[3], (3, 3), padding='same'))
                                self.model.add(Activation('relu'))
    ...
    def run_trainer(self, _X_path, _y_path, depth, model_path, neurons):
        self.load_data(X_path=_X_path, y_path=_y_path)
        self.process_data(100, 200)
        self.create_model(depth, neurons)
        self.compile_model()
